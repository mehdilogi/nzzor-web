...
// Paginated hotel search. Unlike getHotels() above this returns the pagination
// envelope as well as the rows, and it deliberately does NOT fall back to mock
// data — a listing page silently showing ten fake hotels instead of an error is
// worse than an empty state.
//
// WHY THIS EXISTS SEPARATELY
//
// getHotels() returns a bare array and hardcodes limit=50. The /hotels page
// used it and then filtered the result client-side, which meant only the first
// 50 hotels by popularity existed as far as the UI was concerned: filtering by
// a city showed only the hotels from that city that happened to make that top
// 50, and hotels 51+ were unreachable entirely. Filtering and paging belong on
// the server, which routes/hotels.js already supports in full.
//
// Called from the server for the first page, and from the browser for "show
// more" and for restoring an accumulated list after a back-navigation.
export async function getHotelsPaged({
  lang = "en", city, stars, q, sort, maxPrice, minPrice, tags, page = 1, limit = 24,
} = {}) {
  ...
  if (tags && (Array.isArray(tags) ? tags.length : tags)) {
    params.set("tags", Array.isArray(tags) ? tags.join(",") : String(tags));
  }
  ...
  try {
    ...
    return {
      ...
      pagination: json.pagination || {
        page: 1, limit, total: (json.data || []).length, totalPages: 1,
      },
    };
  } catch {
    return { hotels: [], pagination: { page: 1, limit, total: 0, totalPages: 0 }, error: true };
  }
}
...
// Create a booking. Returns { ok, data?, account?, error?, code? }.
// On success, `account` is non-null only when the user opted to create
// an account at booking time and the backend actually did so (the email
// wasn't already registered). Shape: { created: true, token, userId }.
// The UI uses this to persist the token + refresh AuthContext so the
// customer lands on the confirmation page already signed in.
...
export async function createBooking(payload) {
  try {
    ...
    return { ok: true, data: json.data, account: json.account || null };
  } catch (err) {
    if (err.message === "no-api") {
      ...
      const ref =
        ...
        "-" +
        Math.random().toString(36).slice(2, 6).toUpperCase();
      return { ok: true, data: { reference: ref, status: "CONFIRMED", demo: true }, account: null };
    }
...
// ---------------------------------------------------------------------------
// PAYMENTS — SATIM-EPG (CIB WEB)
// ---------------------------------------------------------------------------

// Register the booking as an order with SATIM and get back the hosted payment
// page URL. Returns { ok, formUrl?, paymentId?, error?, code? }.
//
// The customer must be sent to `formUrl` EXACTLY as returned — never to a URL
// we build ourselves. SATIM serves the payment form from a different host than
// the API, so any reconstructed URL would be a guess.
//
// There is deliberately NO demo/mock fallback here. Inventing a payment URL
// would be far worse than failing loudly: the customer would either land
// nowhere or believe they had paid when they had not.
export async function startSatimPayment(reference, { lang = "fr" } = {}) {
  try {
    const json = await apiFetch(`/api/payments/satim/initiate`, {
      method: "POST",
      body: JSON.stringify({ reference, lang }),
    });
    const formUrl = json?.data?.formUrl;
    if (!formUrl) {
      return { ok: false, error: "The payment gateway did not return a payment page.", code: "NO_FORM_URL" };
    }
    return { ok: true, formUrl, paymentId: json.data.paymentId };
  } catch (err) {
    return {
      ok: false,
      error: err.message === "no-api" ? "Payments are not available right now." : err.message,
      code: err.code || null,
    };
  }
}

// Read a booking's payment state without contacting SATIM. Used by the
// /booking/result page so a refresh shows the settled state.
export async function getPaymentStatus(reference) {
  try {
    const json = await apiFetch(`/api/payments/satim/status/${reference}`);
    return json.data || null;
  } catch {
    return null;
  }
