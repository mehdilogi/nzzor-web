...
// ---- TODAY'S ACTIVITY ----
// Powers the "Today" panel on the admin overview tab. Returns counts of
// bookings created/confirmed/cancelled today plus a short list of recent
// state transitions, all evaluated in Algiers local time.
export async function adminToday() {
  const json = await authFetch("/api/admin/today");
  return json.data;
}
...
// ---- ROOM BOARD / MEAL-PLAN RATES (Phase B) ----
// Per-room pricing for each board type (Room-only / BB / DP+ / PC+ /
// All-Inclusive). adminSetBoardRates sends the full set; the backend upserts
// boards with a price and removes those sent with price:null (replace
// semantics). payload shape: { rates: [{ board, price, isActive }, ...] }.
export async function adminGetBoardRates(roomId) {
  const json = await authFetch(`/api/admin/rooms/${roomId}/board-rates`);
  return json.data;
}
export async function adminSetBoardRates(roomId, payload) {
  const json = await authFetch(`/api/admin/rooms/${roomId}/board-rates`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
...
// Promote a photo to "primary" — the one used as the hotel card thumbnail
// and the hero image on the hotel detail page. The backend transactionally
// clears the flag on all sibling photos and sets it on this one, so the
// hotel always has exactly one primary photo.
export async function adminSetPrimaryPhoto(photoId) {
  return authFetch(`/api/admin/photos/${photoId}`, {
    method: "PATCH",
...
// ---- CUSTOMERS (admin view of customer user accounts) ----
// List with pagination + search. Returns { data, pagination }.
export async function adminUsers(params = {}) {
  const qs = new URLSearchParams(params).toString();
  const json = await authFetch(`/api/admin/users${qs ? `?${qs}` : ""}`);
  return json;
}
// Detail with booking history. Returns the user object including
// `bookings: Booking[]`.
export async function adminUserDetail(id) {
  const json = await authFetch(`/api/admin/users/${id}`);
  return json.data;
}
