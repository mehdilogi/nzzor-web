...
<style>
  /* Force browsers to print background colors. By default Chrome/Safari/etc.
     drop background fills when printing to save ink, which would strip the
     red brand dot, the status pill, the cream page background, etc. The
     -webkit prefix covers Chrome/Edge/Safari; the standard property covers
     Firefox. This is the only browser-supported way to opt back in. */
  * {
    box-sizing: border-box;
    -webkit-print-color-adjust: exact;
    print-color-adjust: exact;
  }
  /* Belt and braces: also set it on @page rules for some print drivers. */
  @media print {
    body, .dot, .status, .v {
      -webkit-print-color-adjust: exact;
      print-color-adjust: exact;
  ...
  /* Red brand dot — must survive printing. */
  .dot {
    width: 26px;
    height: 26px;
    border-radius: 50%;
  ...
  .status { display: inline-block; margin-top: 8px; padding: 4px 12px; border-radius: 980px; background: #E7F4EC; color: #1B8A5A; font-size: 11px; font-weight: 700; letter-spacing: 0.04em; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
