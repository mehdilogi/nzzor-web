...
// Dictionary key is the wilaya KEY (lowercase, no accents — matches the DB
// city column after normalize-on-save). The matched values are the aliases
// the user might type (EN / FR / AR spellings + transliterations).
//
// Returning the key (not the display name) is what makes the filter actually
// work: URL ?city=setif matches DB city='setif'. The display name "Sétif" is
// resolved separately at render time via NAME_BY_KEY below.
//
// Adding/renaming wilayas is a single-edit: update WILAYAS in lib/wilayas.js
// and both the AI parser and the homepage picker pick it up.
import { WILAYAS } from "./wilayas";

const CITY_TERMS = Object.fromEntries(
  WILAYAS.map((w) => [w.key, w.aliases])
);

// Map from key -> display name, so consumers (the AI banner) can render
// "Sétif" while the URL uses the clean key "setif".
const NAME_BY_KEY = Object.fromEntries(
  WILAYAS.map((w) => [w.key, w.name])
);
...
export function parseQuery(rawQuery) {
  ...
  if (city) {
    filter.city = city;
    matched.push({ type: "city", value: city, displayName: NAME_BY_KEY[city] || city });
  }
